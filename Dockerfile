...
# Temporary workaround for old alpha color SCSS
RUN find /app/www/app/javascript/ -name "*.scss" -type f -exec sed -E -i 's/rgba\((.*),\s*(0\.[0-9]*)\);/color.change(\1, $alpha: \2);/g' {} +
# TODO:  To use color.mix() with non-legacy color oklch(...), you must provide a $method.
